...
describe("[Challenge] Climber", function () {
  ...
  it("Exploit", async function () {
    // Help from forum: https://forum.openzeppelin.com/t/uupsupgradeable-vulnerability-post-mortem/15680
    // The bug is in ClimberTimelock.execute
    // Anyone can execute what has been scheduled via ClimberTimelock.schedule
    // It first executes the operation
    // Then it checks if the operation is in ready state
    // So operations that are not in ready state can be executed on behalf of ClimberTimelock.address
    // If only it is scheduled
    // First ensure that the contract has the proposer role
    // Second upgrade the proxy to new logic address
